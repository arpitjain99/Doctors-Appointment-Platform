...
// Removed invalid/unused imports:
// import { generateTestToken, generateUserId } from "@/lib/zegocloud-config";
...
export async function bookAppointment(formData) {
  ...
  try {
    ...
    // Create a Zego room ID and store it
    const roomId = createVideoRoomId(doctorId, patient.id);
    ...
    const appointment = await db.appointment.create({
      data: {
        ...
        patientDescription,
        status: "SCHEDULED",
        videoSessionId: roomId, // Zego roomId stored here
      },
    });
...
/**
 * Generate a ZegoCloud room ID for video session
 */
function createVideoRoomId(doctorId, patientId) {
  return `appointment_${Date.now()}_${doctorId}_${patientId}`;
}
